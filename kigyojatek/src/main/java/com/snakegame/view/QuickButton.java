...
public class QuickButton extends Canvas {
    ...
    /**
     * Two booleans to test if the mouse is down and to see if the
     * enablebutton()/disablebutton() method was called by the parent applet
     **/
     ...
     * QuickButton Constructor takes - String label - text which is the button's
     * label - String font - Font to construct the label - int fontsize - Size
     * of font - Color c - Color to use as foreground of label
     **/
    public QuickButton(String label, String font, int fontsize, Color c) {
        this.label = label;
        this.font = font;
    ...
    public Dimension minimumSize() {
        // must over-ride this, or the layout manager may not function as
        // desired
        return QuickButtonsize;
    }

    /**
     * Creates the peer. This peer allows you to change the user interface of
     * the canvas without changing its functionality.
     **/
    public void addNotify() {
        super.addNotify();
        // Button size is dependent (by our choice) on the fontmetrics of the
        // label and its font.
        QuickButtonsize = measureMe();
    }

    /**
     * Calculates the dimensions that are required by the particular label using
     * the particular font. Then calculates the button size accordingly.
     **/
    ...
    protected void paintNormalButton(Graphics g) {
        ...
        int fonttotalheight = g.getFontMetrics().getHeight() + g.getFontMetrics().getLeading();
    ...
    /**
     * Paints the Disabled/DOWN State QuickButton Here we are basically changing
     * the button and label background to a darker shade and shifting the label
     * by 3 pixels
     **/
    ...
    /**
     * If mouse is down on a button and a mouse up follows It means that user
     * wants to 'click' our button. So we need to 'post' a event to the parent
     **/
