...
public class Board extends JPanel {
    ...
    public void paint(Graphics g) {
        ...
        // System.out.println("pause el�tt");
        drawPause(g2d);
    ...
    public void drawFood(Graphics2D g2d) {
        ...
        if (snakegame.isShowfood2())
            g2d.drawImage(sp.BIARRAY[SnakePic.PIC.FOOD2.ordinal()],
        ...
        for (int k = 1; k < getSnakegame().getSnake().size() - 1; k++) {
            ...
            if (getSnakegame().getSnake().get(k).getPartKind() == SnakeBodyPart.Parts.NEWPART) {
                ...
                g2d.drawImage(sp.BIARRAY[20], point.x + getSnakegame().getSnake().get(k).getX() * squarepixsize + 1,
                        point.y + getSnakegame().getSnake().get(k).getY() * squarepixsize + 1, null);
                // System.out.println(getSnakegame().getSnake().get(k).getX()+"
                // x y"+getSnakegame().getSnake().get(k).getY());
            } else

            if (isSlam(getSnakegame().getSnake().get(k - 1), getSnakegame().getSnake().get(k + 1))) {
    ...
    public SnakePic.PIC whichSlam(SnakeBodyPart before, SnakeBodyPart current, SnakeBodyPart after) {
